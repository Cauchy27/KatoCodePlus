export default Sounds = {
  rightStraight:require('./sound/right-straight.mp3'),
  gaia:require('./sound/gaia.mp3'),
  yokero:require('./sound/pokemon2.mp3'),
  drill:require('./sound/mach-drill.mp3'),

  voice1:require('./sound/voice1.mp3'),
  yatteru:require('./sound/yatteru-.mp3'),
  yaruo:require('./sound/yaruo.mp3'),

  bgm1:require('./sound/bgm/cryagain.mp3'),
  bgm2:require('./sound/bgm/kirinukiChamp-member.mp3'),
  bgmGeneMenu:require('./sound/bgm/KatomonGenerateSlow.mp3'),

  battle1:require('./sound/bgm/battle1.mp3'),
  battle2:require('./sound/bgm/battle2.mp3'),
  battle3:require('./sound/bgm/battle3.mp3'),
  battle4:require('./sound/bgm/champ2.mp3'),
  battle5:require('./sound/bgm/casino.mp3'),

  gameOver:require('./sound/bgm/gameOver.mp3'),
  next1:require('./sound/fujinami2.mp3'),

  champ4:require('./sound/bgm/champ4.mp3'),

  gear1:require('./sound/gear1.mp3'),
  gear2:require('./sound/gear2.mp3'),

  hiroyuki1:require('./sound/hiroyuki1.mp3'),
  hiroyuki2:require('./sound/hiroyuki2.mp3'),
  hiroyuki3:require('./sound/hiroyuki3.mp3'),

  moko1:require('./sound/o-n.mp3'),

  sepia:require('./sound/sepia1-poisson.mp3'),

  katou1:require('./sound/voice1.mp3'),
  katou2:require('./sound/katou-ryumai.mp3'),
  katou3:require('./sound/katou-isogasi.mp3'),
  katou4:require('./sound/katou-baka-game.mp3'),
  katou5:require('./sound/tetuya1.mp3'),
  katou6:require('./sound/fujinami4.mp3'),
  katou7:require('./sound/pubg1-1.mp3'),
  katou8:require('./sound/fujinami3.mp3'),

  katomonGenerate:require('./sound/tetuya2.mp3'),
  generate1:require('./sound/generate1.mp3'),

  kenia:require('./sound/kenia.mp3'),
  say:require('./sound/say.mp3'),
  arigatone:require('./sound/arigatone.mp3'),
  thanks:require('./sound/thank_you_so_much.mp3'),
}